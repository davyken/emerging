...
export function Watch() {
  ...
  return (
    ...
    >
      ...
      {/* Top bar */}
      <div
        className="absolute top-0 left-0 right-0 flex items-center px-6 h-14 transition-opacity duration-300 z-20"
        style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.8), transparent)', opacity: showControls ? 1 : 0 }}
        onClick={(e) => e.stopPropagation()}
      >
        <img src="/logo.png" alt="EmergingStream" className="mr-8 flex-shrink-0" style={{ height: '22px', width: 'auto' }} />
        {['Accueil', 'Films', 'Séries TV', 'En direct', 'Ma Liste'].map((item, i) => (
          <a key={item} href="#" className="text-xs font-medium mr-5 transition-colors" style={{ color: i === 1 ? 'var(--color-gold)' : 'rgba(255,255,255,0.5)' }}>{item}</a>
        ))}
        <div className="flex-1" />
        <Link to="/" className="flex items-center gap-2 mr-4" onClick={(e) => e.stopPropagation()}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="2"><path d="M19 12H5 M12 5l-7 7 7 7" /></svg>
          <span className="text-sm font-medium text-white">{media?.title ?? 'Les Ombres de la Frontière : Partie II'}</span>
        </Link>
        <span className="text-xs" style={{ color: '#666' }}>S1 : E4 • 48 min restantes</span>
      </div>
      ...
      {showSettings && (
        <div
          className="absolute right-6 bottom-20 rounded-xl z-30 p-4 w-64"
          style={{ background: 'rgba(15,15,15,0.95)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(16px)' }}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-white">Paramètres de lecture</span>
          ...
          <div className="mb-3 pb-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex-1">
                <p className="text-xs font-medium text-white">Qualité</p>
              </div>
              <select
                value={currentLevel}
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs font-medium text-white">Sous-titres</p>
              </div>
      ...
      <div
        ...
        style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.9), transparent)', opacity: showControls ? 1 : 0, paddingBottom: '1rem' }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Progress bar */}
        ...
        <div className="flex items-center px-6 gap-4">
          ...
          <button className="text-gray-400 hover:text-white transition-colors" onClick={() => { if (videoRef.current) videoRef.current.currentTime = Math.max(videoRef.current.currentTime - 10, 0) }}>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => { const v = videoRef.current; if(v) { v.muted = !v.muted; setMuted(v.muted) }}} className="text-gray-400 hover:text-white transition-colors">
              {muted || volume === 0
                ...
                : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5" /><path d="M15.54 8.46a5 5 0 0 1 0 7.07 M19.07 4.93a10 10 0 0 1 0 14.14" /></svg>
              }
            </button>
            <input type="range" min={0} max={1} step={0.05} value={muted ? 0 : volume}
              ...
              className="w-20 h-1 cursor-pointer accent-white" style={{ accentColor: 'white' }}
            />
          </div>
          ...
          <button onClick={(e) => { e.stopPropagation(); setShowSettings(v => !v) }} className="transition-colors" style={{ color: showSettings ? 'var(--color-gold)' : 'rgba(255,255,255,0.6)' }}>
